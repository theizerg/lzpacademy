    ...
    <div class="col-lg-12">
      <div class="card card-line-primary">
            <div class="card-header">
              <h4 class="card-title">Listado de los periodos escolares activos</h4>
            </div>
            <div class="card-body">
                ...
                <table id="tableExport" class="display table table-striped table-hover" >
                  ...
                  <tbody>
                    @foreach ($periodos as $element)
                      <tr>
                        <td>{{ $element->id }}</td>
                        <td>{{ $element->nb_periodo }}</td>
                       
                        <td>
                         {{ $element->calendario->nb_calendario }}
                        </td>
                        <td>{{ $element->fe_inicio }}</td>
                        <td>{{ $element->fe_fin }}</td>
                       <td>
                          {{ $element->created_at->diffForHumans()  }} 
                        </td>
                        <td>{{ $element->nu_periodo }}</td>
                         @if ($element->status == 1)
                          <td>
